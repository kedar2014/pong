...
with tf.name_scope('Model'):
    ...
    img1 = tf.reshape(tf.transpose(tf.trainable_variables()[0],None),[-1,80,80,1])
    tf.summary.image('weight_layer1',img1,max_outputs=50)                        


    #W2 = tf.get_variable(name="W2",shape=2,dtype=tf.float32)                        
    output = tf.layers.dense(ly1,3,use_bias=False,
...
with tf.Session() as sess:
    ...
    while True:
        ...
        observation, reward, done, info = env.step(action[0][0]+1)   # 1 - Stay, 2 - Up, 3 - Down
        #if reward == 1 :print ('ep %d: game finished, reward: %f' % (episode_number, reward)) 
